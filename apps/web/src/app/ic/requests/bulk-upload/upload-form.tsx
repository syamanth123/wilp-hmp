...
import { bulkCreateHandoutsAction } from './actions';
import type { BulkUploadResult } from './bulk-create';
...
export function BulkUploadForm() {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      ...
      fd.set('csv', text);
      const r = await bulkCreateHandoutsAction(fd);
      setResult(r);
    });
  };
    ...
    <div className="space-y-4">
      ...
      {result?.status === 'success' && (
        <div className="space-y-2" data-testid="bulk-success">
          <p className="text-sm text-emerald-600">
            Created {result.created} handout request{result.created === 1 ? '' : 's'} (status
            REQUESTED).
          </p>
      ...
      {result?.status === 'partial' && (
        <div className="space-y-2" data-testid="bulk-partial">
          <p className="text-sm text-emerald-600">
            Created {result.created} request{result.created === 1 ? '' : 's'}.
          </p>
          <RefNoList refNos={result.refNos} />
          <div className="rounded-md border p-3" style={{ borderColor: 'var(--warn)' }}>
            <p className="text-sm font-medium">
              {result.strandedAtDraft.length} request
              {result.strandedAtDraft.length === 1 ? '' : 's'} created but still at DRAFT
            </p>
            <p className="text-muted-foreground text-xs">
              These were inserted but their move to REQUESTED failed. Open each from the requests
              list to retry.
            </p>
            <ul className="mt-1 text-xs">
              {result.strandedAtDraft.map((s) => (
                <li key={s.refNo} className="font-mono">
                  {s.refNo} — {s.reason}
                </li>
              ))}
            </ul>
      ...
      {result?.status === 'rejected' && (
        <div className="space-y-3" data-testid="bulk-rejected">
          <p className="text-destructive text-sm font-medium">
            ...
            {result.errors.length === 1 ? '' : 's'} found. Nothing was created. Fix the rows below
