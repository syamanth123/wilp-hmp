...
function BookIcon()  { return <svg aria-hidden="true" focusable="false" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 5a2 2 0 012-2h12v18H6a2 2 0 01-2-2V5z"/><path d="M4 17h14"/></svg>; }
...
const GROUPS = [
  ...
  { label: 'In progress',          statuses: [HandoutStatus.ASSIGNED, HandoutStatus.IN_PROGRESS, HandoutStatus.REWORK_REQUESTED] },
  { label: 'Forwarded / Done',     statuses: [HandoutStatus.UNDER_REVIEW, HandoutStatus.APPROVED, HandoutStatus.PUBLISHED] },
];
...
export default async function PCOverview() {
  ...
  const statusColor = (active: boolean) => active ? { color: '#1e6a40', bg: '#e7f5ec', border: '#c9e5d3' } : { color: '#8b5a07', bg: '#fef3e0', border: '#f3dba5' };
  ...
  return (
    <div>
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          <h1 style={{ fontFamily: 'var(--font-serif)', fontWeight: 600, fontSize: 30, letterSpacing: '-0.015em', margin: 0 }}>Programme Committee</h1>
          <p style={{ color: 'var(--muted)', marginTop: 4, fontSize: 14 }}>Confirm assignments and run the review loop.</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="Awaiting assignment" value={counts[0]?.count ?? 0} sub="HOG allocated"       icon={<ClockIcon />} />
        <StatCard label="Awaiting review"     value={counts[1]?.count ?? 0} sub="Faculty submitted"   icon={<BookIcon />} />
        <StatCard label="In progress"         value={counts[2]?.count ?? 0} sub="Faculty working"     icon={<FlowIcon />} />
        <StatCard label="Forwarded / Done"    value={counts[3]?.count ?? 0} sub="Passed PC this term" icon={<CheckIcon />} />
      </div>

      <SlaWidgets role="PC" />

      {/* Programme structure table */}
      <div className="glass-panel rounded-[10px] overflow-hidden mt-5">
        <div className="panel-head"><h3>Programme structure</h3></div>
        <div style={{ padding: 0 }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'rgba(238,242,251,0.7)' }}>
                {['Code', 'Programme', 'Status'].map(h => (
                  <th key={h} style={{ textAlign: 'left', padding: '10px 16px', fontSize: 11.5, fontWeight: 600, color: 'var(--ink-2)', textTransform: 'uppercase', letterSpacing: '0.04em', borderBottom: '1px solid var(--border)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {programmes.map(p => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: '11px 16px', fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink-2)' }}>{p.code}</td>
                    <td style={{ padding: '11px 16px', fontWeight: 600, fontSize: 13.5 }}>{p.name}</td>
                    <td style={{ padding: '11px 16px' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 8px', borderRadius: 999, fontSize: 11.5, fontWeight: 600, background: sc.bg, color: sc.color, border: `1px solid ${sc.border}` }}>
