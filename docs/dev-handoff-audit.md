...
| AI token-cost columns | [apps/web/src/app/admin/ai-metrics/page.tsx:208-211](../apps/web/src/app/admin/ai-metrics/page.tsx) | The AI-metrics page renders real per-day usage queries (see §7-A — not a stub). The actual gap is that `promptTokens` / `completionTokens` aren't persisted to indexed columns — they live nested in `AIDraftLog.payload` JSON and aren't captured at all on `AIRecommendation` / `AIQualityReport`. The in-page footer admits this verbatim: *"Per-call token counts are not persisted yet — cost rollup will be added once the AI provider returns and we capture token usage per request."* | Schema migration to promote token columns + wire `client.chatJson` to return + persist token counts. Tracked as §8 item 1. |
| BullMQ workers | README previously claimed "BullMQ + Redis queues" — corrected in this PR | The cron route runs sweeps synchronously via HTTP. **No Redis queue producer/consumer exists in the repo** (verified via `rg "bullmq\|@bullmq" apps packages`). All notification delivery is in-band inside the user's request cycle. | Either wire actual BullMQ jobs (+ a worker process) or remove the queue claim from architecture docs. README rewrite in this PR removes the false claim. |
