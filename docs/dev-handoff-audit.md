...
| 8 | **Prompt 8 wording-regression guardrail** — when Prompt 8 seeds the `handout.sme_nominated` notification template, the seeded template's `subject` and `body` MUST equal or improve on the inline fallback wording in [apps/web/src/lib/notifications.ts](../apps/web/src/lib/notifications.ts) (`notifySmeNomination` function). The fallback is what real recipients see in production between Prompt 6 landing and Prompt 8 landing, so the template must not introduce a copy regression. Diff the two before merging Prompt 8. | Prompt 6 → 8 handoff | **Pending — verify during Prompt 8 review.** |
