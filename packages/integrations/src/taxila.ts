...
// ---------------------------------------------------------------------------
// LEGACY STUB — removed in Prompt 9b.
//
// The real engine above (publishToLms) has ZERO production callers in 9a. The
// IC publish action keeps calling this preserved stub so behavior is byte-
// identical to before this PR (m6a e2e still passes, no MinIO/Taxila needed).
// 9b rewires publishAction to the real publishToLms and deletes everything
// below this line.
// ---------------------------------------------------------------------------

export interface StubPublishInput {
  handoutId: string;
  versionNo: number;
  contentHtml: string;
  courseCode: string;
...
export interface StubPublishResult {
  status: 'success' | 'failed';
  responseJson: Record<string, unknown>;
}

/** @deprecated Legacy simulated publish. Removed in Prompt 9b once publishAction
 *  is wired to the real two-mode `publishToLms`. */
export async function publishToLmsStub(input: StubPublishInput): Promise<StubPublishResult> {
  return {
    status: 'success',
    responseJson: {
