...
describe('corpus parser — Module template (F3)', () => {
  itIfReady('returns SKIPPED_MODULE with data: null', async () => {
    const r = await parseDocxFile(F3);
    expect(r.extractionMethod).toBe('SKIPPED_MODULE');
    expect(r.data).toBeNull();
  });

  itIfReady('best-effort extracts the course number for admin filtering', async () => {
    const r = await parseDocxFile(F3);
    expect(r.bitsCourseNumber).toBe('EE ZG999');
  });

  itIfReady('warns that 11f-b will address Module templates', async () => {
    const r = await parseDocxFile(F3);
    expect(r.warnings.some((w) => /Course Modules template/i.test(w))).toBe(true);
    expect(r.warnings.some((w) => /11f-b/i.test(w))).toBe(true);
  });
});
